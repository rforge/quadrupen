...
// #include "group_l1linf.h"

// using namespace Rcpp ;
// using namespace arma ;

// SEXP group_l1linf(SEXP N, SEXP X, SEXP XTY, SEXP PK, SEXP S, SEXP LAMBDA, SEXP WEIGHTS, SEXP XBAR, SEXP YBAR, SEXP NORMX, SEXP EPS, SEXP ZERO, SEXP MAXITER, SEXP MAXFEAT, SEXP FUN, SEXP VERBOSE, SEXP SPARSE) {
...
//   arma::mat  R                                             ; // Cholesky decomposition of XAtXA
...
//     grd_norm = grp_norm(grd, pk, 1, 1) - pen;
...
// //       if (fun == 1) {
// // 	if (grpnorm == -1) {
// // 	  out_optim = pathwise_grpl1linf(betaA, pk.elem(GA), xAtxA, xty.elem(A), xtxw, pen.elem(A), gamma, eps);
// // 	}
// // 	if (grpnorm == 2) {
...
// //       }
//       if (fun == 2) {
// 	out_optim = fista_grp(betaA, pk.elem(GA), xAtxA, xty.elem(A), pen.elem(A), -1, L0, eps, zero);
// 	L0 = as<double>(out_optim[2]);
//       }
//       if (fun == 0) {
// 	// not working yet...
// 	// out_optim = quadra_grpl1linf(betaA, pk.elem(GA), R, xty.elem(A), pen.elem(A), zero);
//       }
...
//       grd = -xty + xtxA * betaA;
//       null = arma::sort(arma::find(abs(grp_norm(betaA, pk.elem(GA), -1, 0) + grp_norm(grd.elem(A) - weights.elem(A) * lambda(m), pk.elem(GA), -1, 0))/p < zero),1) ;
//       if (!null.is_empty()) {
